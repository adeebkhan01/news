...
// Columns added after snapshot_stories first shipped. `headline` froze
// whichever language the lead article happened to be written in, with no
// translated counterpart — so an archived day always showed a mix of
// English and Bangla headlines, regardless of the language toggle, the same
// bug the live feed solved for articles long ago (title/title_en/title_bn).
// CREATE TABLE IF NOT EXISTS does not add columns to a table that already
// exists, so a real migration is needed for the databases already on disk.
var SNAPSHOT_STORIES_MIGRATIONS = [
  { column: 'lang',        ddl: "ALTER TABLE snapshot_stories ADD COLUMN lang TEXT" },
  { column: 'headline_en', ddl: "ALTER TABLE snapshot_stories ADD COLUMN headline_en TEXT" },
  { column: 'headline_bn', ddl: "ALTER TABLE snapshot_stories ADD COLUMN headline_bn TEXT" },
  // 'general' or 'broker' (the AU for-brokers briefing). NULL on every row
  // written before this existed, which readMode()'s COALESCE treats as
  // 'general' rather than needing a backfill — the two modes were the same
  // thing until broker mode existed.
  { column: 'mode',        ddl: "ALTER TABLE snapshot_stories ADD COLUMN mode TEXT" }
];

function migrateSnapshotStories(db) {
  var existing = db.prepare('PRAGMA table_info(snapshot_stories)').all()
    .map(function (c) { return c.name; });
  SNAPSHOT_STORIES_MIGRATIONS.forEach(function (m) {
    if (existing.indexOf(m.column) === -1) db.exec(m.ddl);
  });

  // Rows written before this migration (and any row whose lead article has
  // since been re-fetched) have a NULL lang — backfill them from the
  // article the headline was frozen from, when that article link still
  // exists. Cheap once caught up: after the first pass this UPDATE matches
  // nothing, since every row writeRun inserts from here on already carries
  // its own lang/headline_en/headline_bn.
  db.exec(
    'UPDATE snapshot_stories SET ' +
    'lang = (SELECT lang FROM articles WHERE articles.link = snapshot_stories.lead_link), ' +
    'headline_en = (SELECT title_en FROM articles WHERE articles.link = snapshot_stories.lead_link), ' +
    'headline_bn = (SELECT title_bn FROM articles WHERE articles.link = snapshot_stories.lead_link) ' +
    'WHERE lang IS NULL AND EXISTS ' +
    '(SELECT 1 FROM articles WHERE articles.link = snapshot_stories.lead_link)'
  );
}

// briefing_items had no mode concept at all before broker mode — same
// after-the-fact ADD COLUMN as snapshot_stories, above.
function migrateBriefingItems(db) {
  var existing = db.prepare('PRAGMA table_info(briefing_items)').all()
    .map(function (c) { return c.name; });
  if (existing.indexOf('mode') === -1) db.exec('ALTER TABLE briefing_items ADD COLUMN mode TEXT');
}

// A handful of AU sources (AFG, Australian Broker, MPA, The Adviser, MFAA)
// are mortgage-broker trade press, not general-audience news — every one of
// their articles is broker-relevant almost by definition, but that is
// exactly why they do not belong in the general AU feed: a reader who did
// not opt into "For brokers" did not ask for trade-press headlines. This
// column is how a row says which audience it was fetched for, independent
// of brokerRelevance()'s keyword-based scoring of the general-audience
// sources. NULL (every row written before this existed) reads as 0 —
// general — via COALESCE, same convention as the mode column above.
function migrateArticles(db) {
  var existing = db.prepare('PRAGMA table_info(articles)').all()
    .map(function (c) { return c.name; });
  if (existing.indexOf('broker_only') === -1) {
    db.exec('ALTER TABLE articles ADD COLUMN broker_only INTEGER');
  }
}

// Neither table's PRIMARY KEY (date, position) includes mode — SQLite
// cannot ALTER a primary key, and this schema has no migration mechanism
// that rebuilds a table yet. Giving the broker rows a position range with
// no overlap against general's (0..~20) sidesteps a collision without one:
// position stays a true per-mode ordering, just offset for storage.
var BROKER_POSITION_OFFSET = 1000;

function normalizeMode(mode) {
  return mode === 'broker' ? 'broker' : 'general';
}
...
function ensureSchema(db) {
  ...
  migrateSnapshotStories(db);
  migrateBriefingItems(db);
  migrateArticles(db);
...
function writeRun(db, run) {
  ...
  try {
    ...
    var upsertArticle = db.prepare(
      ...
      'title_en, desc_en, title_bn, desc_bn, translate_failed, story_id, score, topic, broker_only) ' +
      'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ' +
      ...
      'score=excluded.score, topic=excluded.topic, broker_only=excluded.broker_only');
    ...
    var insertBriefing = db.prepare(
      ...
      'headline_bn, what_bn, why_bn, watch_bn, mode) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
    if (run.briefingItems) {
      db.prepare("DELETE FROM briefing_items WHERE date = ? AND COALESCE(mode, 'general') = 'general'")
        .run(run.date);
      run.briefingItems.forEach(function (item, i) {
        ...
        insertBriefing.run(run.date, i, item.id || null, item.headline, item.what, item.why,
          item.watch || '', bn.headline || null, bn.what || null, bn.why || null, bn.watch || null, 'general');
      });
    }
    if (run.brokerBriefingItems) {
      db.prepare("DELETE FROM briefing_items WHERE date = ? AND mode = 'broker'").run(run.date);
      run.brokerBriefingItems.forEach(function (item, i) {
        var bn = (run.brokerBriefingItemsBn && run.brokerBriefingItemsBn[i]) || {};
        insertBriefing.run(run.date, BROKER_POSITION_OFFSET + i, item.id || null, item.headline, item.what,
          item.why, item.watch || '', bn.headline || null, bn.what || null, bn.why || null, bn.watch || null,
          'broker');
      });
    }
    ...
    if (run.snapshotStories || run.brokerSnapshotStories) {
    ...
    var insertSnap = db.prepare(
      ...
      'source_ids, topic, score, why, why_bn, lang, headline_en, headline_bn, mode) ' +
      'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
    if (run.snapshotStories) {
      db.prepare("DELETE FROM snapshot_stories WHERE date = ? AND COALESCE(mode, 'general') = 'general'")
        .run(run.date);
      run.snapshotStories.forEach(function (s, i) {
        insertSnap.run(run.date, i, s.id, s.headline, s.leadLink, s.size,
          ...
          s.lang || null, s.headlineEn || null, s.headlineBn || null, 'general');
      });
    }
    if (run.brokerSnapshotStories) {
      db.prepare("DELETE FROM snapshot_stories WHERE date = ? AND mode = 'broker'").run(run.date);
      run.brokerSnapshotStories.forEach(function (s, i) {
        insertSnap.run(run.date, BROKER_POSITION_OFFSET + i, s.id, s.headline, s.leadLink, s.size,
          JSON.stringify(s.sourceIds), s.topic, s.score, s.why || null, s.whyBn || null,
          s.lang || null, s.headlineEn || null, s.headlineBn || null, 'broker');
      });
    }
...
function readArticles(dbconn, sourceMeta) {
  ...
  return rows.map(function (r) {
    ...
    if (r.broker_only) a.brokerOnly = true;
...
// hide a bug.
function readBriefing(dbconn, date, mode) {
  var rows = dbconn.prepare("SELECT * FROM briefing_items WHERE date = ? AND COALESCE(mode, 'general') = ? ORDER BY position")
    .all(date, normalizeMode(mode));
...
function readLatestBriefing(dbconn, mode) {
  var m = normalizeMode(mode);
  var row = dbconn.prepare("SELECT date FROM briefing_items WHERE COALESCE(mode, 'general') = ? ORDER BY date DESC LIMIT 1")
    .get(m);
  return row ? readBriefing(dbconn, row.date, m) : null;
}
...
function readSnapshotStories(dbconn, date, mode) {
  if (!date) return null;
  return dbconn.prepare("SELECT * FROM snapshot_stories WHERE date = ? AND COALESCE(mode, 'general') = ? ORDER BY position")
    .all(date, normalizeMode(mode))
    .map(function (r) {
      return { id: r.story_id, headline: r.headline, leadLink: r.lead_link, size: r.size,
               ...
               lang: r.lang, headlineEn: r.headline_en, headlineBn: r.headline_bn };
    });
}
