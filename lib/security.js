...
//
// A source marked `aggregator` is the exception: its feed URL is allowlisted
// so it can be fetched, but its domain earns no trust beyond that. Google News
// is the case in point — news.google.com's registrable domain is google.com,
// so treating it like a publisher would put every Google host into the link
// allowlist and *.google.com into the page's img-src. An aggregator is a way
// in, not a publisher, and this is where that distinction is kept.
function buildEgressPolicy(regions) {
  ...
  Object.keys(regions).forEach(function (key) {
    (regions[key].sources || []).forEach(function (s) {
      ...
      feedUrls.add(u.href);
      // An aggregator's own domain earns nothing from carrying a feed. It can
      // still say where the articles live, via linkDomains below — which is
      // the publisher's domain, not the aggregator's.
      if (!s.aggregator) {
